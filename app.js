...
app.post('/api/combos', async (req, res) => {
  const recom = [];
  result = Combination.aggregate([{$match: {$expr: {$cond: {if: {$gt: [{$size: res.body.faves}, 0]}, then: {$and: [{combo: {$all: req.body.aThreshold}}, {combo: {$in: res.body.aThreshold}}]}, else: {combo: {$all: res.body.aThreshold}}}}}}, {$sample: {size: req.body.number}}, ])
