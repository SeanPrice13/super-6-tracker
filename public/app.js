...
function countNum(numberArr) {
  ...
  const occurrences = numberArr.reduce((acc, e) => acc.set(e, (acc.get(e) || 0) + 1), new Map()), [num, numCount] = [[...occurrences.keys()], [...occurrences.values()]];
  // Display the result in appropriate element.
  for (let i = 0; i < document.getElementById("count").querySelectorAll("p").length; i++) {
    const el = document.getElementById("count").querySelectorAll("p")[i], sliceB = num.findIndex((element) => element == el.innerText.slice(0, 2));
